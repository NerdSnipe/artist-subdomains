...
export default function Carousel({
    ...
    return (
        ...
        >
            ...
            {count > 1 && (
                <>
                    {/* Hero mode used to anchor these near the top to stay clear of the
                        bottom-anchored headline/CTA text — but that put them directly under the
                        site header, which sits on top of the hero and swallows the clicks.
                        Leveling them with the position-dots row instead keeps them clear of the
                        header AND out of the text zone (the dots row is already reserved space). */}
                    <button
                        type="button"
                        aria-label="Previous image"
                        onClick={() => go(index - 1)}
                        className={`absolute left-3 md:left-6 z-20 bg-[#E9DFC9] text-[#0C0B09] w-10 h-10 md:w-12 md:h-12 flex items-center justify-center border-2 border-[#E9DFC9] hover:bg-[#C9A227] transition-colors ${
                            overlay === "hero" ? "bottom-4 md:bottom-6" : "top-1/2 -translate-y-1/2"
                        }`}
                    >
                        <ChevronLeft size={22} />
                    </button>
                    <button
                        ...
                        onClick={() => go(index + 1)}
                        className={`absolute right-3 md:right-6 z-20 bg-[#E9DFC9] text-[#0C0B09] w-10 h-10 md:w-12 md:h-12 flex items-center justify-center border-2 border-[#E9DFC9] hover:bg-[#C9A227] transition-colors ${
                            overlay === "hero" ? "bottom-4 md:bottom-6" : "top-1/2 -translate-y-1/2"
                        }`}
                    >
