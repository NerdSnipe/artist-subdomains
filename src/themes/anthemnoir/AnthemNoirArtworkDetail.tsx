        ...
        <div>
                ...
                <Reveal>
                    <div className="relative flex items-center justify-center mb-3">
                        {img && (
                            ...
                            // and no background of its own on this wrapper — it used to carry an
                            // explicit bg-black that didn't quite match the page's #0C0B09, leaving a
                            // faintly visible rectangle once the border was removed. Letting it inherit
                            // the page background instead means there's no seam, ever. A bottom/right
                            // drop shadow gives depth; a dark shadow is invisible against this theme's
                            // near-black background (a dark piece just vanishes into it), so it's a
                            // soft white glow instead.
                            // eslint-disable-next-line @next/next/no-img-element
                            <img
                                src={img}
                                alt={product.title}
                                className="max-w-full max-h-[45vh] sm:max-h-[55vh] md:max-h-[70vh] object-contain shadow-[10px_10px_28px_-6px_rgba(255,255,255,0.35)]"
                            />
                        )}
            ...
            {product.youtubeVideoId && (
                <section className="max-w-[1600px] mx-auto px-5 md:px-10 pb-20 md:pb-28">
                    ...
                    <div className="relative w-full aspect-video border-2 border-[#E9DFC9] overflow-hidden">
                        <iframe
                            src={`https://www.youtube.com/embed/${product.youtubeVideoId}`}
                            title={`${product.title} — video`}
                            className="absolute inset-0 w-full h-full"
