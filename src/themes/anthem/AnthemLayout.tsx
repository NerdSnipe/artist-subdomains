...
export default function AnthemLayout({ children, artist, domain }: ThemeLayoutProps) {
    ...
    return (
        <div className={`${fontVariables} min-h-screen flex flex-col bg-[#F7F4EC] text-black font-[family-name:var(--font-body)] antialiased`}>
            ...
            >
                <div className="max-w-[1500px] mx-auto px-5 md:px-10 h-[88px] flex items-center justify-between">
                    <Link
                        href="/"
                        className={`font-[family-name:var(--font-display)] uppercase tracking-tight text-3xl md:text-4xl leading-none transition-colors ${
                    ...
                    <nav className="hidden lg:flex items-center gap-7">
                        {NAV_LINKS.map((l) => {
                            const isActive = l.href === "/" ? pathname === "/" : pathname?.startsWith(l.href.split("#")[0]) && l.href !== "/";
                            return (
                                <Link
                                    key={l.href}
                                    href={l.href}
                                    className={`text-[15px] font-bold tracking-[0.14em] uppercase pb-1 border-b-2 transition-colors ${
                                        overlay
                                            ? isActive
                        ...
                        <Link
                            href="/contact"
                            className={`text-[13px] font-bold tracking-[0.14em] uppercase px-4 py-2 transition-colors ${
                                overlay
                    ...
                    <button
                        type="button"
                        className={overlay ? "text-white lg:hidden" : "text-black lg:hidden"}
                ...
                <nav
                    className={`lg:hidden overflow-hidden border-t-4 border-black bg-[#F7F4EC] transition-[max-height] duration-300 ease-out ${
                ...
                >
                    <div className="px-5 py-6 flex flex-col gap-5">
                        ...
                        <Link href="/contact" className="text-sm font-bold tracking-[0.14em] uppercase text-black">
                            Contact
                        </Link>
                    </div>
                </nav>
